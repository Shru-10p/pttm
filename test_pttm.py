#!/usr/bin/env python3
import os
os.environ["PMO_CONFIG_PATH"] = "pmo_config.json"
import unittest
import json
import pttm
...
class TestPomodoro(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if os.path.exists(pttm.CONFIG_FILE):
            os.rename(pttm.CONFIG_FILE, self.config_backup_path)
            self.has_backup = True

    def tearDown(self):
        # Remove test configuration file
        if os.path.exists(pttm.CONFIG_FILE):
            os.remove(pttm.CONFIG_FILE)

        # Restore backup if we created one
        if self.has_backup:
            os.rename(self.config_backup_path, pttm.CONFIG_FILE)

    def test_load_default_config(self):
        # When file does not exist, load_config should return a copy of the default structure
        config = pttm.load_config()
    ...
    def test_save_and_load_config(self):
        ...
        pttm.save_config(test_config)

        # Verify it loads back accurately
        loaded = pttm.load_config()
    ...
    def test_make_clock_ascii(self):
        ...
        clock_text = pttm.make_clock_ascii(1500)
        ...
        clock_text_zero = pttm.make_clock_ascii(0)
